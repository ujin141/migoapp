...
export const AdminAnalytics = () => {
  ...
  return <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-foreground flex items-center gap-2">
            <TrendingUp size={24} className="text-primary" />{t("auto.z_\uC0AC\uC6A9\uC790\uBD84\uC11D_1054")}</h1>
          <p className="text-sm text-muted-foreground mt-1">{t("auto.z_DAU\uC2E0\uADDC\uAC00\uC785\uC131\uBCC4\uAD6D_1055")}</p>
        </div>
        <motion.button whileTap={{
        scale: 0.95
      }} onClick={load} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-muted text-muted-foreground text-sm">
          <RefreshCw size={14} className={loading ? "animate-spin" : ""} />{t("auto.z_\uC0C8\uB85C\uACE0\uCE68_1056")}</motion.button>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-2 gap-3">
        {[{
        label: t("auto.z_\uC804\uCCB4\uC720\uC800_1057"),
        value: stats.users || 0,
        icon: Users,
        color: "from-blue-500 to-cyan-500"
      }, {
        label: t("auto.z_\uC624\uB298\uD65C\uC131\uC720\uC800DAU_1058"),
        value: dau,
        icon: TrendingUp,
        color: "from-emerald-500 to-teal-500"
      }, {
        label: t("auto.z_\uC804\uCCB4\uAC8C\uC2DC\uAE00_1059"),
        value: stats.posts || 0,
        icon: BarChart3,
        color: "from-violet-500 to-purple-600"
      }, {
        label: t("auto.z_\uC5EC\uD589\uADF8\uB8F9_1060"),
      ...
      <div className="bg-card rounded-2xl border border-border p-5">
        <p className="text-sm font-bold text-foreground mb-4">{t("auto.z_\uC8FC\uAC04\uC2E0\uADDC\uAC00\uC785\uADF8\uB8F9\uC0DD\uC131_1061")}</p>
      ...
      {monthly.length > 0 && <div className="bg-card rounded-2xl border border-border p-5">
          <p className="text-sm font-bold text-foreground mb-4">{t("auto.z_\uC6D4\uBCC4\uC2E0\uADDC\uAC00\uC785\uCD5C\uADFC6\uAC1C_1062")}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {gender.length > 0 && <div className="bg-card rounded-2xl border border-border p-5">
            <p className="text-sm font-bold text-foreground mb-4">{t("auto.z_\uC131\uBCC4\uBD84\uD3EC_1063")}</p>
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
        ...
        {nations.length > 0 && <div className="bg-card rounded-2xl border border-border p-5">
            <p className="text-sm font-bold text-foreground mb-3">{t("auto.z_\uAD6D\uC801TOP10_1064")}</p>
