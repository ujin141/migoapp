  ...
  return (
    <div className="truncate">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6 truncate">
        {[
          { label: t("auto.g_1287", "긴급SOS"), value: sosCount, color: "from-red-500 to-rose-600", icon: AlertTriangle, urgent: sosCount > 0 },
          { label: t("auto.g_1288", "활성체크인"), value: activeCount, color: "from-blue-500 to-cyan-500", icon: MapPin, urgent: false },
          { label: t("auto.g_1289", "처리완료"), value: resolvedCount, color: "from-emerald-500 to-green-500", icon: CheckCircle, urgent: false },
        ].map(s => (
          <div key={s.label} className={`bg-card rounded-2xl p-4 border flex items-center gap-4 ${s.urgent ? "border-red-500/50 shadow-[0_0_20px_rgba(239,68,68,0.15)]" : "border-border"}`}>
            ...
            {s.urgent && <span className="ml-auto px-2 py-1 rounded-full bg-red-500/10 text-red-400 text-[10px] font-bold animate-pulse truncate">⚠️ {t("auto.g_1290", "긴급SOS")}</span>}
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-2 mb-4 truncate">
        {[
          { id: "all" as const, label: t("auto.g_1291", "전체") },
          { id: "sos" as const, label: `🆘 SOS (${sosCount})` },
          { id: "active" as const, label: t("auto.g_1292", "활성") },
          { id: "resolved" as const, label: t("auto.g_1293", "처리완료") },
        ].map(f => (
      ...
      {loading ? (
        ...
        <div className="space-y-3 truncate">
                ...
                <div className="flex items-center gap-4 truncate">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1 truncate"><MapPin size={10} />{c.location_name || t("auto.g_1297", "위치없음")}</span>
          ...
          {filtered.length === 0 && (
            <div className="py-20 text-center text-sm text-muted-foreground truncate">
              {filter === "sos"
                ? t("auto.g_1299", "긴급SOS없음")
                : filter === "resolved"
                ? t("auto.g_1300", "처리완료없음")
                : t("auto.g_1301", "체크인없음")}
            </div>
          )}
        </div>
        ...
        {selected && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.div className="relative z-10 w-full max-w-md bg-card rounded-3xl shadow-float border border-border p-6" initial={{ scale: 0.95 }} animate={{ scale: 1 }}>
              ...
              <div className="space-y-3 text-sm truncate">
                <div className="flex justify-between">
                  <span className="text-muted-foreground truncate">{t("auto.g_1303", "상태")}</span>
                ...
                {selected.latitude && <div className="flex justify-between">
                  <span className="text-muted-foreground truncate">{t("auto.g_1308", "좌표")}</span>
                  <span className="font-mono text-xs">{selected.latitude?.toFixed(4)}, {selected.longitude?.toFixed(4)}</span>
                </div>}
                <div className="flex justify-between">
                  <span className="text-muted-foreground truncate">{t("auto.g_1309", "체크인시각")}</span>
