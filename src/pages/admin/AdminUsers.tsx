...
export const AdminUsers = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setLoadError(i18n.t("auto.z_\uC720\uC800\uBAA9\uB85D\uC744\uBD88\uB7EC\uC624\uC9C0\uBABB_883", "\uC720\uC800\uBAA9\uB85D\uC744\uBD88\uB7EC\uC624\uC9C0\uBABB") + e.message + ")");
      setUsers([]);
    } finally {
      setLoading(false);
    }
  ...
  return <div className="truncate">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-foreground truncate">{t("auto.g_1314", "유저관리5")}</h1>
          <p className="text-sm text-muted-foreground truncate">{t("auto.g_1315", "총")}{users.length}{t("auto.g_1316", "명등록실시")}</p>
        </div>
        ...
        scale: 0.95
          ...
          <RefreshCw size={14} className={loading ? "animate-spin" : ""} />{t("auto.g_1317", "새로고침5")}
        </motion.button>
      </div>
      ...
      {loadError && <div className="mb-4 px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/30 flex items-center justify-between gap-3">
          ...
          <button onClick={load} className="shrink-0 text-xs font-bold text-red-500 underline">{t("auto.z_\uC7AC\uC2DC\uB3C4_892", "\uC7AC\uC2DC\uB3C4")}</button>
        </div>}
      ...
      <div className="flex gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t("auto.g_1318", "이름이메일")} className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-muted border border-border text-sm text-foreground placeholder:text-muted-foreground outline-none focus:border-primary transition-colors" />
        </div>
      ...
      {/* 테이블 (3열) */}
      <div className="bg-card rounded-2xl border border-border overflow-hidden truncate">
        {loading ? <div className="py-16 flex items-center justify-center gap-3 text-muted-foreground">
            <div className="w-5 h-5 border-2 border-primary border-t-transparent rounded-full animate-spin" />
            <span className="text-sm truncate">{t("auto.g_1319", "불러오는중")}</span>
            ...
            setNoteEdit(u.admin_note || "");
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1 truncate">
                          <p className="text-sm font-bold text-foreground truncate">{u.name || t("auto.g_1320", "이름없음5")}</p>
                          {u.banned && <span className="shrink-0 px-1.5 py-0.5 rounded bg-red-500/10 text-[9px] font-bold text-red-400 truncate">{t("auto.g_1321", "정지")}</span>}
                        </div>
                  ...
                  <td className="px-3 py-3" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center gap-1">
                      <motion.button whileTap={{ scale: 0.9 }} onClick={() => verifyUser(u.id, u.verified)} className={`p-1.5 rounded-lg transition-colors ${u.verified ? "bg-muted text-muted-foreground" : "bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20"}`} title={u.verified ? t("auto.g_1324", "인증취소6") : t("auto.g_1325", "인증승인6")}>
                        <Check size={12} />
                      </motion.button>
                      <motion.button whileTap={{ scale: 0.9 }} onClick={() => cyclePlan(u.id, u.plan, u.is_plus)} className={`p-1.5 rounded-lg transition-colors ${u.plan === 'premium' ? "bg-purple-500/20 text-purple-500" : u.plan === 'plus' || u.is_plus ? "bg-amber-500/20 text-amber-500" : "bg-amber-500/10 text-amber-400 hover:bg-amber-500/20"}`} title={i18n.t("auto.z_\uD50C\uB79C\uC21C\uD658FreePl_907", "\uD50C\uB79C\uC21C\uD658FreePl")}>
                        <Crown size={12} />
                      </motion.button>
                      <motion.button whileTap={{ scale: 0.9 }} onClick={() => toggleBan(u.id, u.banned)} className={`p-1.5 rounded-lg transition-colors ${u.banned ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400 hover:bg-red-500/20"}`} title={u.banned ? t("auto.g_1326", "정지해제6") : t("auto.g_1327", "계정정지6")}>
                        <Ban size={12} />
                      </motion.button>
                    </div>
                  </td>
        ...
        {!loading && filtered.length === 0 && <div className="py-12 text-center text-sm text-muted-foreground truncate">{t("auto.g_1328", "유저가없습")}</div>}
      </div>
        ...
        opacity: 0
          ...
          stiffness: 300
              ...
              <div className="p-6 truncate">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-extrabold text-foreground truncate">{t("auto.g_1329", "유저상세6")}</h3>
                ...
                {selectedUser.bio && <div className="mb-6 p-3 bg-muted/40 rounded-xl">
                    <p className="text-[11px] text-muted-foreground font-bold mb-1 truncate">{t("auto.g_1337", "자기소개6")}</p>
                ...
                <div className="space-y-2">
                  <p className="text-xs font-bold text-muted-foreground mb-2 truncate">{t("auto.g_1342", "관리액션6")}</p>
                  <motion.button whileTap={{
                scale: 0.97
                    ...
                    <Check size={14} />{selectedUser.verified ? t("auto.g_1343", "인증취소6") : t("auto.g_1344", "인증승인6")}
                  </motion.button>
                  <motion.button whileTap={{
                scale: 0.97
                    ...
                    <Crown size={14} />{selectedUser.plan === 'premium' ? t("auto.z_Premium\uB4F1\uAE09_927", "Premium\uB4F1\uAE09") : selectedUser.plan === 'plus' || selectedUser.is_plus ? t("auto.z_Plus\uB4F1\uAE09_928", "Plus\uB4F1\uAE09") : t("auto.z_Free\uB4F1\uAE09\uD074\uB9AD\uD558\uC5EC_929", "Free\uB4F1\uAE09\uD074\uB9AD\uD558\uC5EC")}
                  </motion.button>
                  <motion.button whileTap={{
                scale: 0.97
