...
export const AdminGroups = () => {
  ...
  return <div className="truncate">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-foreground truncate">{t("auto.z_\uC5EC\uD589\uADF8\uB8F9\uAD00\uB9AC_863", "\uC5EC\uD589\uADF8\uB8F9\uAD00\uB9AC")}</h1>
          <p className="text-sm text-muted-foreground truncate">{t("auto.z_\uCD1D_864", "\uCD1D")}{groups.length}{t("auto.z_\uAC1C\uD65C\uC131\uADF8\uB8F9\uC2E4\uC2DC\uAC04DB_865", "\uAC1C\uD65C\uC131\uADF8\uB8F9\uC2E4\uC2DC\uAC04DB")}</p>
        </div>
        <motion.button whileTap={{
        scale: 0.95
      }} onClick={load} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-muted text-muted-foreground text-sm">
          <RefreshCw size={14} className={loading ? "animate-spin" : ""} />{t("auto.z_\uC0C8\uB85C\uACE0\uCE68_866", "\uC0C8\uB85C\uACE0\uCE68")}</motion.button>
      </div>

      {/* Summary */}
      <div className="grid grid-cols-3 gap-4 mb-6 truncate">
        {[{
        label: t("auto.z_\uC804\uCCB4\uADF8\uB8F9_867", "\uC804\uCCB4\uADF8\uB8F9"),
        value: groups.length,
        icon: Plane,
        color: "from-blue-500 to-cyan-500"
      }, {
        label: t("auto.z_Premium\uADF8\uB8F9_868", "Premium\uADF8\uB8F9"),
        value: groups.filter(g => g.is_premium).length,
        icon: Crown,
        color: "from-amber-400 to-yellow-500"
      }, {
        label: t("auto.z_\uCD1D\uCC38\uC5EC\uC790_869", "\uCD1D\uCC38\uC5EC\uC790"),
        value: totalMembers,
        icon: Users,
      ...
      {/* Search */}
      <div className="mb-4">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t("auto.z_\uADF8\uB8F9\uBA85\uD638\uC2A4\uD2B8\uBAA9\uC801\uC9C0\uAC80_870", "\uADF8\uB8F9\uBA85\uD638\uC2A4\uD2B8\uBAA9\uC801\uC9C0\uAC80")} className="w-full px-4 py-2.5 rounded-xl bg-muted border border-border text-sm text-foreground placeholder:text-muted-foreground outline-none focus:border-primary transition-colors" />
      </div>

      {loading ? <div className="py-16 flex items-center justify-center gap-3 text-muted-foreground">
          <div className="w-5 h-5 border-2 border-primary border-t-transparent rounded-full animate-spin" />
          <span className="text-sm truncate">{t("auto.z_\uBD88\uB7EC\uC624\uB294\uC911_871", "\uBD88\uB7EC\uC624\uB294\uC911")}</span>
        </div> : filtered.length === 0 ? <div className="py-16 text-center text-sm text-muted-foreground truncate">{t("auto.z_\uC0DD\uC131\uB41C\uC5EC\uD589\uADF8\uB8F9\uC774\uC5C6\uC2B5_872", "\uC0DD\uC131\uB41C\uC5EC\uD589\uADF8\uB8F9\uC774\uC5C6\uC2B5")}</div> : <div className="grid grid-cols-2 gap-4 truncate">
          {filtered.map(g => {
        ...
        return <motion.div key={g.id} layout className="bg-card rounded-2xl p-5 border border-border">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{i18n.t("auto.z_\uD638\uC2A4\uD2B8_873", "\uD638\uC2A4\uD2B8")}{g.hostName}</p>
                  </div>
                  <motion.button whileTap={{
              scale: 0.9
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground mb-3 flex-wrap">
                  <span className="flex items-center gap-1 truncate">
                    <Users size={10} />{memberCount}/{maxMembers > 0 ? maxMembers : "?"}{i18n.t("auto.z_\uBA85_875", "\uBA85")}</span>
                  <span className="flex items-center gap-1 truncate"><Plane size={10} />{g.destination || i18n.t("auto.z_\uBBF8\uC815_876", "\uBBF8\uC815")}</span>
              ...
              width: `${fillPct}%`
                ...
                <div className="flex justify-between text-[10px] text-muted-foreground mt-1">
                  <span className="truncate">{i18n.t("auto.z_\uCC38\uC5EC\uC728_877", "\uCC38\uC5EC\uC728")}</span>
