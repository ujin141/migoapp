...
const ProductModal = ({
  ...
  const [form, setForm] = useState<Partial<Package>>(pkg || {
    ...
    category: "tour",
    price: 0,
    duration: i18n.t("auto.g_1232", "1일"),
    maxPeople: 10,
    currentPeople: 0,
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div className="relative z-10 w-full max-w-lg bg-card rounded-3xl p-6 shadow-float max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-xs font-bold text-muted-foreground mb-1 block">{i18n.t("auto.g_1235", "상품명")}</label>
            <input value={form.title} onChange={e => setForm({
            ...form,
            title: e.target.value
          })} className="w-full bg-muted rounded-xl px-4 py-3 text-sm outline-none" placeholder={i18n.t("auto.g_1236", "예오사카먹")} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs font-bold text-muted-foreground mb-1 block">{i18n.t("auto.g_1242", "가격원")}</label>
            ...
            <div>
              <label className="text-xs font-bold text-muted-foreground mb-1 block">{i18n.t("auto.g_1243", "목적지")}</label>
              <input value={form.destination} onChange={e => setForm({
              ...form,
              destination: e.target.value
            })} className="w-full bg-muted rounded-xl px-4 py-3 text-sm outline-none" placeholder={i18n.t("auto.g_1244", "예오사카일")} />
            </div>
            <div>
              <label className="text-xs font-bold text-muted-foreground mb-1 block">{i18n.t("auto.g_1245", "소요시간기")}</label>
              <input value={form.duration} onChange={e => setForm({
              ...form,
              duration: e.target.value
            })} className="w-full bg-muted rounded-xl px-4 py-3 text-sm outline-none" placeholder={i18n.t("auto.g_1246", "예4시간또")} />
            </div>
          </div>
          ...
          <div>
            <label className="text-xs font-bold text-muted-foreground mb-1 block">{i18n.t("auto.g_1248", "대표이미지")}</label>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <label htmlFor="featured" className="text-sm font-semibold text-foreground">{i18n.t("auto.g_1249", "인기Fea")}</label>
          </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-foreground flex items-center gap-2 truncate">
            <Store size={24} className="text-primary" />{t("auto.g_1252", "마켓상품관")}</h1>
          <p className="text-sm text-muted-foreground mt-1 truncate">{t("auto.g_1253", "투어액티비")}</p>
        </div>
        <button onClick={() => {
        setEditingItem(null);
        setShowModal(true);
      }} className="px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm flex items-center gap-2">
          <Plus size={16} />{t("auto.g_1254", "새상품등록")}</button>
      </div>

      <div className="bg-card rounded-2xl border border-border overflow-hidden truncate">
        {loading ? <div className="p-12 text-center text-muted-foreground text-sm font-semibold truncate">{t("auto.g_1255", "상품목록을")}</div> : items.length === 0 ? <div className="p-12 text-center text-muted-foreground text-sm font-semibold truncate">{t("auto.g_1256", "등록된상품")}</div> : <div className="overflow-x-auto">
              ...
              <tbody className="divide-y divide-border">
                {items.map(item => <tr key={item.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-4 text-sm font-bold text-foreground text-right">
                      {item.price.toLocaleString()}{t("auto.g_1267", "원")}</td>
