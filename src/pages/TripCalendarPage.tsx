...
const getPalette = (t: any) => [{
  color: "#f43f5e",
  label: t("trip.colorRose")
}, {
  color: "#f97316",
  label: t("trip.colorOrange")
}, {
  color: "#6366f1",
  label: t("trip.colorIndigo")
}, {
  color: "#10b981",
  label: t("trip.colorEmerald")
}, {
  color: "#8b5cf6",
  label: t("trip.colorPurple")
}, {
  color: "#0ea5e9",
  label: t("trip.colorSky")
}];
const EMOJIS = ["✈️", "🏖️", "🏔️", "☕", "🎭", "🏄", "🌸", "🌿", "🎒", "🍜"];
const AddTripModal = ({
  onClose,
  ...
  return <motion.div className="fixed inset-0 z-[70] flex items-end justify-center px-safe pb-safe pt-safe" initial={{
    ...
    opacity: 0
      ...
      stiffness: 300
        ...
        <h3 className="text-lg font-extrabold text-foreground mb-4">{t("trip.newTrip")}</h3>
  ...
  return <div className="min-h-screen bg-background safe-bottom pb-24">
      ...
      <header className="flex items-center gap-3 px-5 pt-12 pb-4">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-extrabold text-foreground">{t("tripCalendar.title")}</h1>
          <p className="text-xs text-muted-foreground mt-0.5">{t('trip.subtitle')}</p>
        </div>
        <motion.button whileTap={{
        scale: 0.92
      ...
      {tab === "calendar" && <div className="px-5 space-y-4">
          ...
          {/* My trip list */}
          <div>
            <h3 className="text-sm font-extrabold text-foreground mb-2 px-1">{t("trip.myTrips", {
            count: trips.length
          })}</h3>
            {trips.length === 0 ? <div className="bg-card rounded-3xl p-8 text-center shadow-card">
                <div className="text-4xl mb-2">🗺️</div>
                <p className="text-sm text-muted-foreground">{t("trip.noTrips")}</p>
      ...
      {/* ── OVERLAP TAB ── */}
      {tab === "overlap" && <div className="px-5 space-y-3">
          <div className="bg-gradient-to-r from-primary/10 to-accent/10 rounded-2xl p-4 border border-primary/20">
            <div className="flex items-center gap-2 mb-1">
              <Sparkles size={14} className="text-primary" />
              <p className="text-xs font-extrabold text-primary">{t("trip.overlapTitle")}</p>
            </div>
            <p className="text-xs text-muted-foreground">{t("trip.overlapDesc")}</p>
          </div>
        ...
        y: 0
              ...
              color: "#6366f1"
                ...
                <div className="flex items-center gap-1 mt-1.5">
                  ...
                  <span className="text-[11px] font-bold text-primary">{t("trip.overlapDays", {
          ...
          <div className="bg-card rounded-2xl p-4 shadow-card">
            <p className="text-xs font-bold text-muted-foreground mb-3">{t("trip.monthAvail")}</p>
