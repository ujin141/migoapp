...
const IdUploadModal = ({
  ...
  const idTips = [i18n.t('verif.id.tip1', "빛반사가없"), i18n.t('verif.id.tip2', "어두운배경"), i18n.t('verif.id.tip3', "주민등록번")];
      ...
      damping: 25
        ...
        {step === "upload" && <>
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep("guide")} className="flex-1 py-3 rounded-2xl border border-border text-foreground font-semibold text-sm">{i18n.t('verif.id.prev')}</button>
            <motion.button whileTap={{
            scale: 0.97
        ...
        {step === "pending" && <div className="flex flex-col items-center py-8 gap-4">
            ...
            <p className="text-base font-bold text-foreground truncate">{i18n.t('verif.id.pendingTitle')}</p>
            <p className="text-sm text-muted-foreground text-center truncate">{i18n.t('verif.id.pendingDesc')}</p>
          </div>}
      </motion.div>
    </motion.div>;
};
