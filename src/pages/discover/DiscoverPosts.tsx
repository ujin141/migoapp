...
const PostCard = memo(
  ({ post, index, isRead, user, handleStoryClick, deletePost }: PostCardProps) => {
    return (
      <motion.div
        initial={{ opacity: 0, y: 15 }}
        animate={{ opacity: 1, y: 0 }}
        className={`w-full aspect-[4/5] rounded-[24px] cursor-pointer shadow-md group p-[3px] ${
          !isRead ? "bg-gradient-to-tr from-amber-400 via-orange-500 to-pink-500" : "bg-transparent"
        }`}
        ...
        <div className="relative w-full h-full bg-muted rounded-[21px] overflow-hidden">
          ...
          {post.images?.[0] || post.imageUrl ? (
            ...
            <div className="w-full h-full gradient-primary flex items-center justify-center p-6">
              <p className="text-white font-extrabold text-[18px] leading-relaxed drop-shadow-md break-keep whitespace-pre-wrap text-center">
                {post.content}
              </p>
            </div>
          )}
  ...
  (prev, next) => {
    return (
      ...
      prev.post.photo === next.post.photo &&
      prev.post.time === next.post.time &&
...
export const DiscoverPosts = memo(
  ({
    activeTab,
    loadingPosts,
    sortedPosts,
          ...
          >
            {loadingPosts ? (
              <div className="flex items-center justify-center py-16">
                <motion.div
                  className="w-8 h-8 rounded-full gradient-primary"
                  animate={{
                    scale: [1, 1.2, 1]
                  }}
                  transition={{
              ...
              <div className="text-center py-16">
                ...
                <p className="text-muted-foreground text-sm text-center">
                  {i18n.t("auto.g_1393", "Be the first to post!")}
                </p>
              </div>
            ) : (
