...
export const distanceLabel = (km: number): string => {
  if (km < 0.1) return i18n.t("auto.g_0324", "50m이내");
...
export const travelTimeLabel = (km: number): string => {
  if (km < 0.3) return i18n.t("auto.g_0325", "도보3분");
  if (km < 1) {
...
export const meetabilityLabel = (km: number): string => {
  if (km < 1) return i18n.t("auto.g_0326", "지금당장만");
  if (km < 5) return i18n.t("auto.g_0327", "가까운거리");
  if (km < 15) return i18n.t("auto.g_0328", "이동하면만");
  return i18n.t("auto.g_0329", "좀멀지만만");
};
...
// Google Maps URL 생성
export const googleMapsUrl = (a: GeoPosition, b: GeoPosition): string => `https://www.google.com/maps/dir/?api=1&origin=${a.lat},${a.lng}&destination=${b.lat},${b.lng}&travelmode=walking`;
...
const useGeoDistance = () => {
  ...
  const requestLocation = useCallback(async () => {
    ...
    if (pos) {
      ...
      setError(i18n.t("auto.g_0330", "이브라우저"));
