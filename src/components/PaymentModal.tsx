...
const PaymentModal = ({
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      toast({
        title: i18n.t("auto.z_\uACB0\uC81C\uC5D0\uC2E4_21683e"),
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const ModalHeader = () => <>
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-border/30 shrink-0">
        <div>
          <h2 className="text-base font-extrabold text-foreground">{i18n.t("auto.z_\uADF8\uB8F9\uCC38\uC5EC_a22379")}</h2>
          ...
          opacity: 1
            ...
            duration: 0.5
                ...
                <div className="text-center">
                  <p className="text-xl font-extrabold text-foreground">{i18n.t("auto.z_\uACB0\uC81C\uC644\uB8CC_0f4cb5")}</p>
                  <p className="text-sm text-muted-foreground mt-1">{groupTitle}{i18n.t("auto.z_\uC5D0\uCC38\uC5EC\uC911_55fe95")}</p>
                </div>
              </motion.div>}
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 min-h-0">
                  ...
                  <div className="rounded-2xl overflow-hidden border border-border">
                    <div className="flex items-center justify-between px-4 py-3 bg-muted/40">
                      <span className="text-sm text-muted-foreground">{i18n.t("auto.z_\uCC38\uC5EC\uBE44\uC6A9_b4522d")}</span>
                    ...
                    {hasDiscount && <div className="flex items-center justify-between px-4 py-3 bg-primary/5 border-t border-primary/20">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold text-primary">{i18n.t("auto.z_Migo_79a69f")}</span>
                        </div>
                  ...
                  <div className="space-y-2">
                    <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest px-1">{i18n.t("auto.z_\uCC38\uC5EC\uBE44\uC6A9_9565c9")}</p>
                ...
                return <div key={tc.tier} className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${isCurrent ? `${tc_colors.bg} ${tc_colors.border}` : "bg-muted/30 border-border"}`}>
                          ...
                          <div className="flex-1">
                            <div className={`text-xs font-bold ${isCurrent ? tc_colors.text : "text-muted-foreground"}`}>
                              ...
                              {isCurrent && <span className="ml-2 text-[9px] px-1.5 py-0.5 rounded-full bg-primary text-white">{i18n.t("auto.z_현재_e7755c", {defaultValue: "Current"})}</span>}
                            </div>
                            <div className="text-[10px] text-muted-foreground">{TIER_LOCALES[i18n.language.split("-")[0] || "en"]?.tier?.[tc.tier]?.tagline || tc.tagline}</div>
                          </div>
                          <div className="text-right">
                            ...
                            {isPlus && <div className="text-[10px] text-primary font-bold">
                  ...
                  <div className="flex gap-2 p-3 rounded-xl bg-rose-500/5 border border-rose-500/20">
                    ...
                    <p className="text-[10px] text-rose-500/80 leading-relaxed">{i18n.t("auto.z_\uCC38\uC5EC\uBE44\uC6A9_8a392c")}<strong>{i18n.t("auto.z_\uB178\uC1FC\uBC29\uC9C0_00e7a3")}</strong>{i18n.t("auto.z_\uB97C\uC704\uD55C\uBCF4_42f050")}</p>
                  </div>
                </div>
            ...
            {step === "select" && <>
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 min-h-0">
                  ...
                  <p className="text-xs font-bold text-muted-foreground">{i18n.t("auto.z_\uACB0\uC81C\uC218\uB2E8_4963ee")}</p>
                  <div className="space-y-2">
                ...
                <div className="px-5 py-4 border-t border-border/30 bg-card shrink-0 flex gap-3">
                  <button onClick={() => setStep("info")} className="flex-1 py-3.5 rounded-2xl bg-muted text-foreground font-bold text-sm">{i18n.t("auto.z_\uC774\uC804_cb1dc9")}</button>
                  <motion.button whileTap={{
              scale: 0.97
            }} onClick={() => setStep("confirm")} className="flex-1 py-3.5 rounded-2xl gradient-primary text-white font-extrabold text-sm flex items-center justify-center gap-1.5">{i18n.t("auto.z_\uD655\uC778_3ce813")}<ChevronRight size={14} />
                  </motion.button>
                </div>
              </>}
            ...
            {step === "confirm" && <>
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-1 min-h-0">
                  {[["그룹", groupTitle], ["티어", `${tierCfg.emoji} ${tierCfg.label}`], ["결제 수단", PAYMENT_METHODS.find(m => m.id === method)?.label ?? method], ["원래 가격", getLocalizedPrice(originalKrw, i18n.language)], ...(hasDiscount ? [["Plus 할인 (-50%)", `- ${getLocalizedPrice(originalKrw - discountedKrw, i18n.language)}`]] : []), ["최종 결제금액", getLocalizedPrice(discountedKrw, i18n.language)]].map(([label, value], i, arr) => <div key={i} className={`flex items-center justify-between py-3 ${i < arr.length - 1 ? "border-b border-border/40" : ""}`}>
                      <span className="text-xs text-muted-foreground">{label}</span>
                      <span className={`text-xs font-bold ${label === "최종 결제금액" ? colors.text + " text-base" : "text-foreground"}`}>{value}</span>
                    </div>)}
                </div>

                <div className="px-5 py-4 border-t border-border/30 bg-card shrink-0 flex gap-3">
                  <button onClick={() => setStep("select")} className="flex-1 py-3.5 rounded-2xl bg-muted text-foreground font-bold text-sm">{i18n.t("auto.z_\uC774\uC804_cb1dc9")}</button>
                  <motion.button whileTap={{
              scale: 0.97
                    ...
                    {loading ? <><div className="w-4 h-4 rounded-full border-2 border-white/40 border-t-white animate-spin" />{i18n.t("auto.z_\uACB0\uC81C\uC911_ce536a")}</> : <><Check size={14} strokeWidth={3} />{getLocalizedPrice(discountedKrw, i18n.language)}{i18n.t("auto.z_\uACB0\uC81C_a4ba01")}</>}
                  </motion.button>
                </div>
              </>}
          </motion.div>
