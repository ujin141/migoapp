...
const SwipeCard = ({
  ...
  // 호환성 점수 세부 계산
  const compatDetails = useMemo(() => {
    if (!isTop || !myProfile) return null;
    const myStyles: string[] = Array.isArray(myProfile.travel_style) ? myProfile.travel_style : Array.isArray(myProfile.interests) ? myProfile.interests : [];
    const pStyles: string[] = Array.isArray(profile.travelStyle) ? profile.travelStyle : Array.isArray(profile.interests) ? profile.interests : [];
    const myLangs: string[] = Array.isArray(myProfile.languages) ? myProfile.languages : [];
    const pLangs: string[] = Array.isArray(profile.languages) ? profile.languages : [];
    const styleScore = myStyles.length && pStyles.length ? Math.round(myStyles.filter(s => typeof s === 'string' && pStyles.includes(s)).length / Math.max(myStyles.length, pStyles.length) * 100) : 50;
    const langScore = myLangs.length && pLangs.length ? Math.round(myLangs.filter(l => typeof l === 'string' && pLangs.includes(l)).length / Math.max(myLangs.length, pLangs.length) * 100) : 50;
  ...
  const fateInsight = useMemo(() => {
    if (!isTop) return { label: "", title: "", detail: "", metric: "" };
    ...
    return {
      ...
      detail: pick({
        ...
        ja: `${mood}의 경향이 가까운 사람`,
        zh: `与你的${mood}风格상근의 사람`,
      }),
      metric: `${routeScore}%`,
    };
  }, [isTop, profile, i18n.language, overallMatch]);
  ...
  const getTarotArchetype = () => {
    ...
    const lang = (i18n.language?.split("-")[0] || "ko").toLowerCase();
    const pick = (text: Record<string, string>) => text[lang] || text.en || text.ko;
    const isSpontaneous = mbti.includes("P") || profile.travelMission === "즉흥 번개" || interests.some((i: any) => typeof i === 'string' && i.includes("즉흥"));
    const isFoodie = (typeof profile.travelMission === "string" && profile.travelMission.includes("맛집")) || interests.some((i: any) => typeof i === 'string' && (i.includes("맛집") || i.includes("미식") || i.includes("카페")));
    const isNight = (typeof profile.travelMission === "string" && profile.travelMission.includes("야경")) || interests.some((i: any) => typeof i === 'string' && (i.includes("야경") || i.includes("밤") || i.includes("클럽") || i.includes("술")));
  ...
  const tarot = useMemo(() => {
    if (!isTop) return { title: "", subtitle: "", spark: "", description: "" };
    return getTarotArchetype();
  }, [isTop, profile, i18n.language]);
  ...
  const handleTap = () => {
    ...
    if (profile?.isAd) {
      if (profile.adUrl) {
        import("@capacitor/browser").then(m => m.Browser.open({ url: profile.adUrl, presentationStyle: "fullscreen" })).catch(() => {});
      }
      return;
    }
    if (isBlurTarget) {
  ...
  return <>
    <motion.div className="absolute inset-0 cursor-grab active:cursor-grabbing" style={{
      ...
      >
        ...
        {/* Full Screen Image */}
        {currentPhoto ? <>
            <img src={currentPhoto} alt="Profile" className={`absolute inset-0 w-full h-full object-cover pointer-events-none ${isTop && isBlurTarget ? 'blur-2xl scale-110 brightness-75' : ''}`} draggable="false" loading={isTop ? "eager" : "lazy"} decoding="async" onError={(e) => {
              (e.target as HTMLImageElement).style.display = 'none';
              e.currentTarget.parentElement?.classList.add('gradient-primary');
            }} />
            {isTop && isBlurTarget && <div className="absolute inset-0 bg-black/40 flex flex-col justify-center items-center z-10 pointer-events-none">
                <Crown size={48} className="text-amber-400 mb-4 drop-shadow-[0_0_15px_rgba(251,191,36,0.5)]" />
                <p className="text-white font-black text-xl drop-shadow-lg text-center px-4 truncate">{i18n.t("auto.z_\uB098\uB7C0\uC88B\uC544\uC694\uD588\uC5B4\uC694_1223", "\uB098\uB7C0\uC88B\uC544\uC694\uD588\uC5B4\uC694")}</p>
          ...
          <div className="absolute bottom-0 left-0 right-0 z-40 p-6 pt-12 flex flex-col gap-2 pointer-events-none">
            <div className="flex justify-between items-end gap-2">
               <div className="flex flex-col flex-1 min-w-0">
                  ...
                  {profile.isLiker && !isBlurTarget && <div className="mt-2 text-[11px] font-bold text-amber-300 flex items-center gap-1.5 drop-shadow pointer-events-none truncate">
    ...
    {isTop && showDetail && <ProfileDetailSheet profile={profile} onClose={() => setShowDetail(false)} onLike={onSwipeRight} showActions={true} />}
