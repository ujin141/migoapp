...
const DEMO_NOW_CARDS: NowCard[] = [{
  ...
  photo: "",
  age: 26,
  location: i18n.t("auto.g_0163", "시부야"),
  activity: i18n.t("auto.g_0164", "라멘같이먹"),
  ...
  id: "now2",
  userId: "u2",
  name: i18n.t("auto.g_0165", "현준"),
  photo: "",
  age: 29,
  location: i18n.t("auto.g_0166", "도톤보리"),
  activity: i18n.t("auto.g_0167", "야경사진같"),
  ...
  photo: "",
  age: 24,
  location: i18n.t("auto.g_0168", "홍대"),
  activity: i18n.t("auto.g_0169", "카페아는곳"),
  ...
  photo: "",
  age: 27,
  location: i18n.t("auto.g_0170", "강남"),
  activity: i18n.t("auto.g_0171", "맛집같이가"),
  ...
  photo: "",
  age: 31,
  location: i18n.t("auto.g_0172", "이태원"),
  activity: i18n.t("auto.g_0173", "치맥같이해"),
...
function useCountdown(expiresAt: number) {
  ...
  const label = remaining <= 0 ? t("auto.g_0174", "만료") : m > 60 ? `${Math.floor(m / 60)}h` : `${m}m`;
  return {
    label,
    pct,
    expired: remaining <= 0
...
const StoryAvatar = ({
  ...
  return <motion.button onClick={onClick} whileTap={{
    scale: 0.93
      ...
      {/* 이름 */}
      <p className="text-[10px] font-bold text-foreground truncate w-full text-center">{isMe ? t("auto.g_0175", "올리기") : card.name}</p>
    ...
    opacity: 0
      ...
      stiffness: 300
        ...
        <div className="flex gap-2">
          <button onClick={onClose} className="flex-1 py-3 rounded-2xl border border-border text-sm font-semibold text-foreground">{t("auto.g_0178", "닫기")}</button>
          ...
          background: `linear-gradient(135deg, ${nearColor}, #6366f1)`
            ...
            <Zap size={14} />{t("auto.g_0179", "지금만나기")}</motion.button>
        </div>
      </motion.div>
    </motion.div>;
};
    ...
    opacity: 0
      ...
      stiffness: 300
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-extrabold text-foreground truncate">{t("auto.g_0180", "지금여기있")}</h3>
            <p className="text-xs text-muted-foreground truncate">{t("auto.g_0181", "2시간동안")}</p>
          </div>
          <button onClick={onClose}><X size={18} className="text-muted-foreground" /></button>
        </div>
        ...
        <div className="space-y-3 mb-4">
          <input value={location} onChange={e => setLocation(e.target.value)} placeholder={t("auto.g_0182", "현재위치예")} className="w-full rounded-2xl bg-muted border border-border px-4 py-2.5 text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/50" />
          <input value={activity} onChange={e => setActivity(e.target.value)} placeholder={t("auto.g_0183", "뭐하고싶어")} className="w-full rounded-2xl bg-muted border border-border px-4 py-2.5 text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/50" />
        </div>
      ...
      }}>{t("auto.g_0184", "지금올리기")}</motion.button>
      </motion.div>
    </motion.div>;
};
  ...
  return <>
      ...
      <div className="flex items-center justify-between px-4 pt-1 pb-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] font-extrabold text-foreground truncate">{t("auto.g_0186", "지금여기있")}</span>
          <span className="text-[10px] text-muted-foreground bg-muted rounded-full px-1.5 py-0.5 ml-0.5 truncate">
            {sortedCards.length}{t("auto.g_0187", "명6")}</span>
        </div>
        <span className="text-[9px] text-muted-foreground truncate">{t("auto.g_0188", "가까운순")}</span>
      </div>
