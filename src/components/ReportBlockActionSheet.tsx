...
const REPORT_REASONS = [] as string[]; // loaded via i18n.t()
...
const ReportBlockActionSheet: React.FC<Props> = ({
  ...
  const handleBlock = async () => {
    if (!user) {
      toast({
        title: i18n.t("reportBlock.loginRequired"),
        variant: "destructive"
      });
      return;
    }
    ...
    toast({
      title: i18n.t("alert.t4Title"),
      description: i18n.t("alert.t4Desc")
    });
    onClose();
  };
  const handleReport = async (reason: string) => {
    ...
    if (error) {
      toast({
        title: i18n.t("alert.t5Title"),
        variant: "destructive"
      });
    } else {
      toast({
        title: i18n.t("alert.t6Title"),
        description: i18n.t("alert.t6Desc")
      });
      onClose();
    }
  };
  return <AnimatePresence>
    {isOpen && <motion.div className="fixed inset-0 z-[500] flex items-end justify-center px-safe pb-safe pt-safe" initial={{
      opacity: 0
    }} animate={{
      opacity: 1
    }} exit={{
      opacity: 0
        ...
        stiffness: 300
            ...
            <div className="px-5 pt-4 truncate">
              ...
              <div className="flex items-center justify-between mb-4 truncate">
                {mode === "report" ? <button onClick={() => setMode("menu")} className="flex items-center gap-1 text-muted-foreground text-sm font-bold">
                    <ChevronRight size={18} className="rotate-180" />{i18n.t("auto.g_0206", "뒤로")}</button> : <h3 className="text-lg font-extrabold text-foreground truncate">
                    {targetType === "user" ? i18n.t("auto.t_0011", `${targetName} 관리`) : i18n.t("auto.g_0207", "게시물 관리")}
                  </h3>}
              ...
              {/* MENU MODE */}
              {mode === "menu" && <div className="space-y-3 truncate">
                  <button onClick={() => setMode("report")} className="w-full flex items-center justify-between p-4 rounded-2xl bg-muted hover:bg-border transition-colors text-left">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-foreground truncate">{i18n.t("auto.g_0208", "신고하기")}</p>
                        <p className="text-xs text-muted-foreground mt-0.5 truncate">{i18n.t("auto.g_0209", "부적절한콘")}</p>
                      </div>
                    </div>
                  ...
                  {(targetType === "user" || authorId) && <button onClick={handleBlock} disabled={isSubmitting} className="w-full flex items-center justify-between p-4 rounded-2xl bg-red-500/5 hover:bg-red-500/10 border border-red-500/10 transition-colors text-left group">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-red-500 truncate">{i18n.t("auto.g_0210", "차단하기")}</p>
                          <p className="text-xs text-red-500/70 mt-0.5 truncate">{i18n.t("auto.g_0211", "서로의활동")}</p>
                        </div>
                      </div>
                    </button>}
                </div>}
              ...
              {mode === "report" && <div className="space-y-3 mt-2">
                  <p className="text-sm font-bold text-foreground mb-4 truncate">{i18n.t("auto.g_0212", "어떤문제가")}</p>
                  <div className="space-y-2 truncate">
                    {(REPORT_REASONS.length > 0 ? REPORT_REASONS : [i18n.t("auto.g_0213", "스팸"), i18n.t("auto.g_0214", "음란/불건전한 콘텐츠"), i18n.t("auto.g_0215", "혐오성 언어"), i18n.t("auto.g_0216", "개인정보 침해"), i18n.t("auto.g_0217", "거짓 정보"), i18n.t("auto.g_0218", "기타")]).map(reason => <button key={reason} onClick={() => handleReport(reason)} disabled={isSubmitting} className="w-full text-left px-4 py-3.5 rounded-xl bg-muted hover:bg-border active:scale-[0.98] transition-all flex items-center gap-3 text-sm font-medium text-foreground">
                        <div className="w-2 h-2 rounded-full bg-amber-500/50" />
                        {reason}
                      </button>)}
                  </div>
                  <p className="text-[10px] text-muted-foreground text-center mt-6 truncate">{i18n.t("auto.g_0219", "허위신고시")}</p>
                </div>}
            </div>
          </motion.div>
        </motion.div>}
