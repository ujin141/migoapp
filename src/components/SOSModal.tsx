...
const EMERGENCY_CONTACTS = [{
  name: i18n.t("auto.g_0220", "경찰청"),
  number: "112",
  icon: "🚔"
}, {
  name: i18n.t("auto.g_0221", "소방서구급"),
  number: "119",
  icon: "🚑"
}, {
  name: i18n.t("auto.g_0222", "여행자보험"),
  number: "1588-0099",
  icon: "🏥"
}];
const SOSModal = ({
  ...
  const [address, setAddress] = useState(i18n.t("sos.locationChecking"));
  ...
  useEffect(() => {
    ...
    getCurrentLocation(false).then(pos => {
      if (pos) {
        ...
        setAddress(i18n.t("auto.g_0223", "위치접근거"));
      }
    });
  }, [isOpen]);
  ...
  const saveSOS = async () => {
    ...
    try {
      ...
      // 레이더 안전 조치: 대상 유저 차단 및 현재 대화방 강제 폭파
      if (targetUserId) {
      ...
      toast({
        ...
        description: i18n.t("alert.t7Desc")
      });
    } catch {
      toast({
        title: i18n.t("alert.t8Title"),
        ...
        stiffness: 300
            ...
            background: "linear-gradient(90deg, #ef4444, #dc2626, #b91c1c)"
              ...
              repeat: Infinity
                  ...
                  <h2 className="text-xl font-extrabold text-white text-center mb-1 truncate">{i18n.t("auto.g_0224", "긴급SOS")}</h2>
                  <p className="text-xs text-red-300/80 text-center mb-5 truncate">{i18n.t("auto.g_0225", "등록된긴급")}</p>
                  <div className="flex items-center gap-2 bg-white/5 rounded-2xl px-4 py-3 mb-4 border border-white/10">
                    <MapPin size={14} className="text-red-400 shrink-0" />
                    <div>
                      <p className="text-xs font-bold text-white truncate">{i18n.t("auto.g_0226", "현재위치")}</p>
            ...
            }}>{i18n.t("auto.g_0227", "지금위치전")}</motion.button>
                  <button onClick={onClose} className="w-full mt-2 py-2 text-xs text-white/40 font-medium">{i18n.t("general.cancel")}</button>
                </div>
              </div>}
            ...
            repeat: Infinity
                ...
                <p className="text-white font-extrabold text-lg mb-1 truncate">{countdown}{i18n.t("auto.g_0228", "초후전송")}</p>
                <p className="text-white/40 text-xs mb-5 truncate">{i18n.t("auto.g_0229", "취소하려면")}</p>
                <button onClick={() => {
            setPhase("warning");
            setCountdown(3);
          }} className="px-6 py-3 rounded-2xl bg-white/10 text-white text-sm font-bold border border-white/20">{i18n.t("auto.g_0230", "취소")}</button>
              </div>}
          ...
          scale: 1
            ...
            boxShadow: "0 0 32px rgba(16,185,129,0.4)"
                ...
                <p className="text-white font-extrabold text-lg mb-1 truncate">{i18n.t("auto.g_0231", "위치가전송")}</p>
                <p className="text-white/50 text-xs mb-5 truncate">{i18n.t("auto.g_0232", "등록된연락")}</p>
                <div className="flex items-center gap-2 bg-emerald-500/10 rounded-2xl px-4 py-3 mb-5 border border-emerald-500/20">
                  <Shield size={14} className="text-emerald-400" />
                  <p className="text-xs text-emerald-300 truncate">{i18n.t("auto.g_0233", "Migo가")}</p>
                </div>
                <button onClick={onClose} className="w-full py-3.5 rounded-2xl bg-white/10 text-white text-sm font-bold border border-white/20">{i18n.t("auto.g_0234", "닫기")}</button>
              </motion.div>}
          </motion.div>
        </motion.div>}
    </AnimatePresence>;
