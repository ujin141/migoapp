  ...
  if (compact) {
    return <button onClick={() => setShowDetail(true)} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-white text-xs font-bold shadow-sm ${className}`} style={{
      background: "var(--gradient-primary)"
        ...
        <span>{emoji}</span>
        <span className="text-[11px]">🔥 {data.currentStreak}{t("auto.z_\uC77C_1229")}</span>
      </button>;
  }
  ...
  return <>
      <button onClick={() => setShowDetail(true)} className={`flex items-center gap-2 px-3 py-1.5 rounded-2xl bg-gradient-to-r ${gradient} shadow-card ${className}`}>
        ...
        <div className="text-left">
          ...
          <p className="text-sm font-black text-white leading-tight">🔥 {data.currentStreak}{t("auto.z_\uC77C\uC5F0\uC18D_1230")}</p>
        </div>
      </button>
      ...
      <AnimatePresence>
        ...
        opacity: 0
          ...
          opacity: 0
              ...
              <p className="text-center text-xs text-muted-foreground mb-5">{t("auto.z_Migo\uC5EC\uD589\uC790\uB4F1\uAE09_1231")}</p>

              {/* 통계 */}
              <div className="grid grid-cols-3 gap-3 mb-5">
                {[{
              label: t("auto.z_\uD604\uC7AC\uC2A4\uD2B8\uB9AD_1232"),
              value: t("auto.z_tmpl_1233", {
                defaultValue: `${data.currentStreak} days`
              }),
              icon: "🔥"
            }, {
              label: t("auto.z_\uCD5C\uC7A5\uC2A4\uD2B8\uB9AD_1234"),
              value: t("auto.z_tmpl_1235", {
                defaultValue: `${data.longestStreak} days`
              }),
              icon: "⭐"
            }, {
              label: t("auto.z_\uCD1D\uBC29\uBB38_1236"),
              value: t("auto.z_tmpl_1237", {
                defaultValue: `${data.totalDays} days`
              }),
              icon: "📅"
              ...
              {next && <div>
                  <div className="flex justify-between text-xs mb-1.5">
                    <span className="text-muted-foreground font-medium">{t("auto.z_\uB2E4\uC74C\uB808\uBCA8_1238")}{next.level}</span>
                    <span className="font-bold text-foreground">{data.currentStreak} / {next.daysRequired}{t("auto.z_\uC77C_1239")}</span>
                  </div>
                ...
                ease: "easeOut"
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1.5 text-center">
                    {next.daysRequired - data.currentStreak}{t("auto.z_\uC77C\uB354\uC5F0\uC18D\uC811\uC18D\uD558\uBA74_1240")}{LEVEL_EMOJI[next.level]} {next.level}{t("auto.z_\uB2EC\uC131_1241")}</p>
                </div>}
              {!next && <p className="text-center text-sm font-bold text-amber-500">{t("auto.z_\uCD5C\uACE0\uB4F1\uAE09\uB2EC\uC131_1242")}</p>}

              <button onClick={() => setShowDetail(false)} className="mt-5 w-full py-3 rounded-2xl bg-muted text-muted-foreground text-sm font-bold">{t("auto.z_\uB2EB\uAE30_1243")}</button>
            </motion.div>
          </motion.div>}
      </AnimatePresence>
    </>;
