...
function vibeEmoji(tags: string[]): string {
  ...
  if ([t("auto.g_0057", "투어"), t("auto.g_0058", "진지"), i18n.t("auto.db_match_history", { defaultValue: "역사" }), t("auto.g_0059", "문화")].some(k => all.includes(k))) return "🎯";
  return "✨";
}
...
const MatchCard: React.FC<MatchCardProps> = ({
  ...
  onClick={handleCardClick}
    className={`bg-card rounded-3xl shadow-card overflow-hidden border border-border/40 ${isTop !== undefined ? "absolute inset-0" : ""} ${onClickCard ? "cursor-pointer" : ""}`}>
      ...
      <div className="relative w-full h-28 overflow-hidden bg-gradient-to-br from-primary/20 to-purple-500/20">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
          <div className="flex gap-1.5 truncate">
            {group.isPremiumGroup && <span className="flex items-center gap-1 text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-amber-400/20 text-amber-400 border border-amber-400/30 backdrop-blur-sm truncate">
                <Crown size={9} />{i18n.t("auto.z_\uAC80\uC99D_0a4c43", "\uAC80\uC99D")}</span>}
            {isPremiumMode && <span className="flex items-center gap-1 text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-primary/20 text-primary border border-primary/30 backdrop-blur-sm truncate">{i18n.t("auto.z_\uD504\uB9AC\uBBF8\uC5C4_5ee37b", "\uD504\uB9AC\uBBF8\uC5C4")}</span>}
          </div>
          {/* Rating */}
      ...
      {/* Body */}
      <div className="p-3.5 space-y-2.5 truncate">
        {/* Match reasons */}
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <span className="flex items-center gap-1 text-[11px] text-muted-foreground truncate">
            <Users size={10} /> {group.currentMembers}/{group.maxMembers}{i18n.t("auto.z_\uBA85_7b3c6e", "\uBA85")}</span>
        ...
        {isPremiumMode && <div className="space-y-1">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest truncate">{i18n.t("auto.z_\uC131\uBE44_e2b321", "\uC131\uBE44")}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="truncate">
            ...
            {group.hostCompletedGroups && group.hostCompletedGroups > 0 && <span className="text-[9px] text-amber-400 font-bold ml-1.5 truncate">🏆 {group.hostCompletedGroups}{i18n.t("auto.z_\uBC88\uC644\uC8FC_f431d0", "\uBC88\uC644\uC8FC")}</span>}
          </div>
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          <div className="flex-1 truncate">
            {isFreeForUser ? <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-extrabold text-pink-500 truncate">{i18n.t("auto.z_\uC5EC\uC131\uBB34\uB8CC_64c6fc", "\uC5EC\uC131\uBB34\uB8CC")}</span>
              </div> : <div>
                <span className="text-xs text-muted-foreground truncate">{i18n.t("auto.z_\uCC38\uC5EC\uBE44_e18045", "\uCC38\uC5EC\uBE44")}</span>
          ...
          <div className="flex gap-2">
            ...
            <motion.button whileTap={{
            scale: 0.95
          }} onClick={(e) => { e.stopPropagation(); onAccept(result); }} className="flex items-center gap-2 px-4 h-10 rounded-xl gradient-primary text-white text-xs font-extrabold shadow-card">{i18n.t("auto.z_\uCC38\uC5EC\uD558\uAE30_0892a2", "\uCC38\uC5EC\uD558\uAE30")}</motion.button>
          </div>
        </div>
      </div>
    </motion.div>;
